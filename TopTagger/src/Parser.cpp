...
void Parser::CompoundIC() {
		Expect(8);
		AssignOrCond();
		while (la->kind == 4 || la->kind == 14) {
			AssignOrCond();
		}
		while (!(la->kind == 0 || la->kind == 9)) {SynErr(25); Get();}
		Expect(9);
}
...
void Parser::ConditionalAssign() {
		Expect(14);
		Expect(15);
		hcal::cfg::TermAnd* terms=new hcal::cfg::TermAnd(); 
		CompoundTerm(*terms);
...
void Parser::Item() {
		Expect(4);
		m_builder.theItemName=CfgBuilder::toString(t->val); m_builder.theItemIndex=-1; 
		while (la->kind == 11) {
			Get();
			Expect(1);
			m_builder.theItemIndex=CfgBuilder::toInt(t->val); 
			while (!(la->kind == 0 || la->kind == 12)) {SynErr(28); Get();}
			Expect(12);
		}
}
...
void Parser::CompoundTerm(hcal::cfg::TermAnd& at) {
		hcal::cfg::SimpleTerm* term=0; 
		Term(term);
		if (term!=0) { at.And(term); term=0; } 
		while (la->kind == 17) {
			Get();
			Term(term);
			if (term!=0) { at.And(term); term=0; } 
...
void Parser::Term(hcal::cfg::SimpleTerm*& term) {
		if (la->kind == 4) {
			Get();
