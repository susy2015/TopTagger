...
namespace hcal {
  namespace cfg {
    ...
    std::auto_ptr<CfgDocument> CfgDocument::parseDocument(const std::string& text, log4cplus::Logger logger) {
      if (text.size()<5) {
	return std::auto_ptr<CfgDocument>(new CfgDocument(logger));
      } else {
	hcalcfg::Scanner sc(text);
	hcalcfg::Parser p(&sc);
	p.m_builder.setup(logger);
	p.Parse();
